/**
 * @file Ast.h
 * @brief Header file for Ast, the base class of any class which represents a node within a given Abstract syntax tree.
 * @version 0.1
 * 
...
// #QUESTION:
      ...
      // the 'visitors'. (meaning we have to add code to check that things are okay to access)
      // as far as i can tell right now, this is about how we have to implement
      // the visitor pattern from the perspective of the Ast.
      // because of:
      // 1) we need some way of communicating the resulting error or type of a typecheck,
      //    the resulting error or value of a codegen, OR the resulting string of a ToString,
      ...
      // also, the whole typechecking algorithm is about as big (maybe even bigger now?) as the parser,
      // so the size of the typechecking file would be big (around 1000 - 1500 lines), and get bigger 
      // as we added new node kinds. so we aren't really reducing the number of massive files in
      // the project, so much as we are removing a large amount of implementation from the 
      // individual files of the Ast.
      // the Ast code as it stands now gets bigger each time we add a new kind 
      // of walk to the tree. just as the typecheckvisitor file gets larger as we 
      ...
      // -) any code sanatizer or formatting function could make use of the visitor pattern
      // as well.
...
namespace pink {
    ...
    protected:
       ...
       */
      /** @property type
       * @brief initially nullptr; however after a call to Getype type caches the [type](#Type) of this Ast node
       */
    ...
    private:
       ...
       * 
       * @return Outcome<Type*, Error> if this Ast could be typed by the type checker, then the Outcome holds that type,
       *                               otherwise the Outcome holds the Error the type checker ran into during evaluation.
       */
    ...
    public:
       ...
       * 
       * \warning Ast is a virtual base class, and so an instance of an Ast cannot be constructed directly,
       * only an instance of a derived class may be constructed.
       * 
       * @param k the [kind](#Kind) of the ast object being constructed
       * @param l the textual [location](#Location) of the ast object being constructed
       */
       ...
       * 
       * @param env The Environment to typecheck against, an [Environment](#Environment) set up as if by [NewGlobalEnv](#NewGlobalEnv) is suitable
       * @return Outcome<Type*, Error> if the type checking algorithm could assign this term a [type](#Type) then the Outcome holds that type,
       *                               otherwise the Outcome holds the Error that the type checking algorithm constructed.
       */
      Outcome<Type*, Error> Getype(const Environment& env);

      /**
       * @brief Computes the LLVM Intermediate Representation ([IR]) corresponding to this Ast.
       * 
       * [IR]: https://llvm.org/docs/LangRef.html "IR"
       * 
       * Internally this function uses the [instruction_builder] to construct the [IR].
       * and does so with respect to the [symbol_table], [binop_table], [unop_table], and [module],
       * held within the environment parameter. (this is not intended as a complete list)
       * 
       * [symbol_table]: @ref SymbolTable "symbol table"
       * [binop_table]: @ref BinopTable "binop table"
       * [unop_table]: @ref UnopTable "unop table"
       * [instruction_builder]: https://llvm.org/doxygen/classllvm_1_1IRBuilder.html "instruction builder"
       * [module]: https://llvm.org/doxygen/classllvm_1_1Module.html "module"
       * 
       * @param env The Environment to generate code against, an [Environment](#Environment) set up as if by [NewGlobalEnv](#NewGlobalEnv) is suitable
       * @return Outcome<llvm::Value*, Error> if the code generator could assign this term a [value] then the Outcome holds this value,
       *                                      otherwise the Outcome holds the [Error] the code generator constructed.
       */
