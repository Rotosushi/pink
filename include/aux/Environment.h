...
namespace pink {
   ...
   * or etc... 
   * These members may simply be shared between the unique instances of the environment.
   ...
   * on each construction, this is code want to place in a function, [NewGlobalEnv](#NewGlobalEnv)
  ...
  public:
    /**
     * @brief the [symbols](@ref InternedString) associated with [bindings](#SymbolTable) constructed during [typechecking](@ref Ast::Getype)
     * 
     ...
     * value may be used in later computation. If we were to not keep track of these
     * symbols, we would still need to solve the problem of how to communicate the 
     * type of a declared symbol to later statements within a scope, when the 
     * typechecking algorithm has no buisness constructing [value] terms.
     * 
     ...
     * within the [CLIOptions](#CLIOptions) in that these flags are not controllable by the 
     * user on the command line, and instead are exclusively set/reset during
     * compilation of specific terms within a given [Ast](#Ast).
     */
     ...
     * 
     * In order to properly represent the options passed to the compiler by the user
     * this member must be constructed via a call to [ParseCLIOptions](#ParseCLIOptions)
     */
    std::shared_ptr<CLIOptions>        options;

    /**
     * @brief The Parser constructs [Ast](#Ast) terms from the associated input stream.
     * 
     * The [parse](#Parser::Parse) routine acts as an inverse operation for [Ast::ToString](#Ast::ToString)
     */
    std::shared_ptr<Parser>            parser;

    /**
     * @brief Interns each [symbol](#InternedString) read by the [parser](#Parser).
     * 
    ...
    /**
     * @brief Interns each [operator](#InternedString) read by the [parser](#Parser).
     * 
     * this structure ensures that we have one and only one copy of any
    ...
    /**
     * @brief Records the [type](#Type) and [value] of each [symbol](#InternedString) which occurs within a 
     *        [bind](#Bind) statement in the source text.
     * 
     * [value]: https://llvm.org/doxygen/classllvm_1_1Value.html "llvm::Value*"
     * 
    ...
    /**
     * @brief Records all Binary Operators ([binop](#InternedString)s) known to the compiler.
     * 
     * This is to record that operator, such that the [parser](#Parser) may know the 
     * [precedence](#Precedence) and [associativity](#Associativity) of the operator, 
     * such that it can construct a [binop](#Binop) term from that operator.
     * 
     */
    std::shared_ptr<BinopTable>        binops;

    /**
     * @brief Records all Unary Operators ([unop](#InternedString)s) known to the compiler.
     * 
     * This is to record that [operator](#InternedString), such that the [parser](#Parser) may construct the 
     * [unop](#Unop) term from that [operator](#InternedString)
     * 
     */
     ...
     * 
     * [context]: https://llvm.org/doxygen/classllvm_1_1LLVMContext.html "llvm::Context"
     * 
     * \warning The [context] *owns* many of the important and useful classes of llvm itself,
     * and as such a new [context] must only be constructed when a new unit of compilation is 
     * similarly constructed. such as when threading the compiler to translate many files 
     * simultaneously
     */
    ...
    /**
     * @brief this is the [IRBuilder] which is used by [codegen] to construct the [instruction]s 
     * corresponding to the [term] being compiled.
     * 
     * [IRBuilder]: https://llvm.org/doxygen/classllvm_1_1IRBuilder.html "llvm::IRBuilder"
     * [codegen]: @ref Ast::Codegen "codegen"
     * [instruction]: https://llvm.org/doxygen/classllvm_1_1Instruction.html "llvm::Instruction"
     * [term]: @ref Ast "term"
     * 
     */
    ...
    /**
     * @brief This is the [Target] which represents some data relevant to the target machine being compiled for.
     * 
     * [Target]: https://llvm.org/doxygen/classllvm_1_1Target.html "Target"
     * 
     * As far as we can tell, this class is constructed when our code calls llvm::InitializeNativeTarget,
     * and is what llvm calls a "managed static" class. From the perspective of our code,
     * this class is mainly used to construct the [TargetMachine] for this unit of compilation.
     * 
     * [TargetMachine]: https://llvm.org/doxygen/classllvm_1_1TargetMachine.html "Target Machine"
     * 
     */
    const llvm::Target*                target;

    /**
     * @brief This is the [TargetMachine] which represents the data relevant to constructing assembly for the 
     ...
     * 
     * [DataLayout]: https://llvm.org/doxygen/classllvm_1_1DataLayout.html "Data Layout"
     * [TargetMachine]: https://llvm.org/doxygen/classllvm_1_1TargetMachine.html "Target Machine"
     * 
     * This class holds information such as the [Endianness] of the [TargetMachine],
     * the sizes of [llvmType]s on the [TargetMachine] and the address spaces for pointers.
     * 
     ...
     * 
     * [llvmFunction]: https://llvm.org/doxygen/classllvm_1_1Function.html "llvm::Function"
     * 
     * Intialized to nullptr, this member can be used to tell if we are 
     * in global scope or a local scope, and as such it is used within [various](#Bind::Codegen)
     * [codegeneration](#While::Codegen) routines to retrieve the [block]s they need to insert 
     * into.
     * 
     ...
     * 
     * [llvmContext]: https://llvm.org/doxygen/classllvm_1_1LLVMContext.html "llvm::LLVMContext" 
     * 
     * The [Module] and [IRBuilder] must be constructed with respect to the [llvmContext] being passed in.
     * 
     * the [Target] must be from the [TargetRegistry], and the [TargetMachine] must be constructed from that [Target].
     * 
     ...
     * [TargetRegistry]: https://llvm.org/doxygen/classllvm_1_1TargetRegistry.html "llvm::TargetRegistry"
     * [TargetMachine]: https://llvm.org/doxygen/classllvm_1_1TargetMachine.html "llvm:TargetMachine"
     * [DataLayout]: https://llvm.org/doxygen/classllvm_1_1DataLayout.html "llvm::DataLayout"
     * 
     * @param flags 
     * @param options 
     * @param parser 
     ...
     * @param current_function The current [llvmFunction] which defines the 'local' Environment. 
     */
    Environment(
      const Environment& env,
      std::shared_ptr<SymbolTable> symbols,
  ...
  /**
   * @brief Constructs a new global Environment. With all members initialized for native codegeneration.
   * 
