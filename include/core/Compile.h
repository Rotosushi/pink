...
/*
   ...
   ground as we can, then shore up the graps left later.
                ...
                We now read the file one line at a time, so we could support
   streaming inputs, but we still buffer everything that we read, and there are
   no provisions in case we exceed available memory for a given file. (yet)
   however one solution would be to buffer all possible input for a given file
   in the same array, and simply shift the input along as it's read in. this
   keeps the memory allocated per file constant for each file being processed.

                The second simplification is that the environment is going to be
   constructed for native code generation only at first. so we are only
   concerned with the native assembly format, the llvm intermidiate
   representation, and object files. all of which can be handled via
 ...
 *  	- if we *could* type the term, then add it to a list of valid terms for
 *        this compilation unit
 * 	- if we *could not* type the term, then report the Error as the
 *        reason and end compilation.
 *      3) Attempt to generate the code from each of the
 *         valid terms constructed by steps 1 and 2
 *      - if we could generate code for the term then move on to the next
 *      - if we could not generate code for the term then report the error
 *        and end compilation
 *
