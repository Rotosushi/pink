...
*/
auto Function::GetypeV(const Environment &env) const -> Outcome<Type *, Error> {
...
auto Function::Codegen(const Environment &env) const
  ...
  and set the local env's ir_builder's insertion point to that.

  then, Codegen the body of the function.

  using the value returned from Codegen'ing the body, construct a return
  statement.
  ...
  */
  ...
  Outcome<llvm::Type *, Error> p_fn_ret_ty_result(
      Error(Error::Code::None, Location()));
  ...
  if (!fn_ret_ty->isSingleValueType() &&
      !function_ty->getReturnType()->isVoidTy()) {
     ...
     */
    for (size_t i = 1; i < function_ty->getNumParams(); i++) {
      llvm::AttrBuilder param_attr_builder(*env.context);

      auto param_ty_codegen_result = p_fn_ty->arguments[i]->Codegen(env);
     ...
     */
    for (size_t i = 0; i < function_ty->getNumParams(); i++) {
      llvm::AttrBuilder param_attr_builder(*env.context);

      Outcome<llvm::Type *, Error> param_ty_codegen_result =
          p_fn_ty->arguments[i]->Codegen(env);
  ...
  llvm::BasicBlock *entryblock =
      llvm::BasicBlock::Create(*env.context, "entry", function);
  ...
  std::shared_ptr<llvm::IRBuilder<>> local_builder =
  ...
  for (size_t i = 0; i < function->arg_size(); i++) {
    ...
    llvm::Value *arg_ptr = local_builder->CreateAlloca(
        arg->getType(), local_env->data_layout.getAllocaAddrSpace(), nullptr,
        arg->getName());
  ...
  for (size_t i = 0; i < arguments.size(); i++) {
    // Outcome<llvm::Type*, Error> param_ty_result =
    // p_fn_ty->arguments[i]->Codegen(env);
    ...
    local_env->bindings->Bind(this->arguments[i].first, p_fn_ty->arguments[i],
                              arg_ptrs[i].second);
  }
  ...
  // now we can Codegen the body of the function relative to the local
  // environment
  ...
  if (std::string(name) == "main") {
    ...
    llvm::FunctionType *iasm0Ty =
        llvm::FunctionType::get(int64Ty, noArgsTy, false);
    llvm::FunctionType *iasm1Ty =
        llvm::FunctionType::get(int64Ty, intArgsTy, false);
    llvm::FunctionType *iasm2Ty =
        llvm::FunctionType::get(voidTy, noArgsTy, false);
    ...
    if (llvm::isa<llvm::ConstantInt>(bodyVal)) {
      if (auto error = llvm::InlineAsm::verify(iasm1Ty, "={rdi},i")) {
        std::string errstr = "constraint code for iasm1Ty not valid: " +
    ...
    Outcome<llvm::Value *, Error> cast_result =
        Cast(bodyVal, local_builder->getInt64Ty(), *local_env);
  ...
  if (llvm::verifyFunction(*function, &out)) {
    // raise the error of a function being
    // unable to be verified to the level of
    // the program exiting. llvm::verifyFunction
    // writes the error to the stream passed in,
    // so we write it to the output here.
