...
namespace pink {
    Assignment::Assignment(const Location& location, std::unique_ptr<Ast> left, std::unique_ptr<Ast> right)
        : Ast(Ast::Kind::Assignment, location), left(std::move(left)), right(std::move(right))
    {
    ...
    */
    auto Assignment::GetypeV(const Environment& env) const -> Outcome<Type*, Error>
    {
    ...
    auto Assignment::Codegen(const Environment& env) const -> Outcome<llvm::Value*, Error>
    {
      ...
      std::string errmsg = std::string("llvm::Value* cannot be written: Value* is -> ")
