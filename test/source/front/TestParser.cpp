...
auto TestParser(std::ostream &out) -> bool {
  ...
  std::vector<std::string> source_lines = {
      ...
      "fn inc(x: Integer) { x + 1; }\n", // function with one argument, integer
                                         // return type
      "fn assign() { x = 4; }\n", // function with no arguments, integer return
                                  // type
      "fn add(x : Integer, y: Integer) { z := x + y; }\n", // function with two
                                                           // arguments, integer
                                                           // return type
      "fn compare(x: Boolean, y: Boolean) { if (x == y) { true; } else { "
      "false; } }\n"};
  ...
  auto [source_locations, teststream] = [&source_lines]() {
    ...
    for (auto &line : source_lines) {
      ...
      // so the lexer does not count it, and this is the expected behavior.
  ...
  result &= Test(out, "Parser::Parse(\"w := 0;\")",
                 ((expr = GetAstOrNull(out, outcome, *env)) != nullptr) &&
  ...
  next_test();
  result &= Test(out, "Parser::Parse(\"fn inc(x: Integer) { x + 1; }\")",
                 ((expr = GetAstOrNull(out, outcome, *env)) != nullptr) &&
  ...
  next_test();
  result &= Test(
      out, "Parser::Parse(\"fn add(x : Integer, y: Integer) { z := x + y; }\")",
      ((expr = GetAstOrNull(out, outcome, *env)) != nullptr) &&
          TestFnBodyIsA<pink::Bind>(expr, location, out));

  next_test();
  result &= Test(out,
                 "Parser::Parse(\"fn compare(x: Boolean, y: Boolean) { if (x "
                 "== y) { true; } else { false; } }\")",
                 ((expr = GetAstOrNull(out, outcome, *env)) != nullptr) &&
                     TestFnBodyIsA<pink::Conditional>(expr, location, out));
