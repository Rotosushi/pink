...
bool TestFunction(std::ostream& out)
{
  ...
  */
  ...
  std::unique_ptr<pink::Function> first = std::make_unique<pink::Function>(fl, fname, fargs, std::move(xv), env->bindings.get());
  
  std::unique_ptr<pink::Function> second = std::make_unique<pink::Function>(sl, sname, sargs, std::move(yv), env->bindings.get());
