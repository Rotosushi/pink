...
bool TestAssignment(std::ostream &out) {
  ...
  */
  ...
  std::unique_ptr<pink::Assignment> asgn =
      std::make_unique<pink::Assignment>(al, std::move(var), std::move(nil));

  result &= Test(out, "Assignment::getKind()",
                 asgn->GetKind() == pink::Ast::Kind::Assignment);
  ...
  result &= Test(out, "Assignment::Getype()",
                 asgn_type && asgn_type.GetFirst() == nil_t);
