...
auto Conditional::TypecheckV(const Environment &env) const
    -> Outcome<Type *, Error> {
  Outcome<Type *, Error> test_type_result = test->Typecheck(env);
  ...
  Outcome<Type *, Error> first_type_result = first->Typecheck(env);

  if (!first_type_result) {
    return first_type_result;
  }

  Outcome<Type *, Error> second_type_result = second->Typecheck(env);
...
auto Conditional::Codegen(const Environment &env) const
  ...
  Outcome<Type *, Error> first_getype_result = first->Typecheck(env);
